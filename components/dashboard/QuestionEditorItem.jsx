...
export default function QuestionEditorItem({
  ...
  const handleQuestionText = (questionId, value) => {
    ...
    newData.find((q) => q.id === questionId).text = value;
  ...
  const handleFileUpload = async (e, questionId, type) => {
    try {
      ...
      setQuestions((prevData) => {
        ...
        const q = newData.find((item) => item.id === questionId);
        if (q && type === 'image') {
          q.image = fileId;
        }
        if (q && type === 'audio') {
  ...
  const handleRemoveFile = (questionId, type) => {
    setQuestions((prevData) => {
      const newData = [...prevData];
      if (type === 'image') {
        newData.find((q) => q.id === questionId).image = null;
      }
      if (type === 'audio') {
        newData.find((q) => q.id === questionId).audio = null;
      }
  ...
  const handleAddOption = (questionId) => {
    setQuestions((prevData) => {
      const newData = prevData.map((q) => {
        let newId = 1;
        while (q.options.find((option) => option.id === newId)) {
          newId++;
        }
        if (q.id === questionId) {
          return {
  ...
  const handleDelete = async (questionId) => {
    try {
      ...
      setQuestions((prevData) => {
        const newData = prevData.filter((q) => q.id !== questionId);
        return newData;
      });
      handleQuestionChange();
    } catch (error) {
      ...
      <div className="flex flex-col gap-3">
        ...
        {!question.audio ? (
          <label
            ...
            className="flex items-center justify-center w-8 h-8 rounded-full bg-whitePlus shadow-right cursor-pointer"
          >
            <img src="/image/audio.svg" alt="" className="scale-75" />
          </label>
        ) : (
          <button
