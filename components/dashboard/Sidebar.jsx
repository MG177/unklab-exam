  ...
  return (
    <div className="relative z-20 h-screen shrink-0">
        ...
        <nav className="flex-1 px-2 py-3">
          ...
          {NAV.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon
                  className={cn('h-[17px] w-[17px] shrink-0', active && 'text-brand')}
                  strokeWidth={2}
                />
            ...
            {!collapsed && (
              <>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-xs font-extrabold text-ink">{name}</div>
      ...
      <button
        ...
        className="absolute right-0 top-[88px] z-30 grid h-6 w-6 translate-x-1/2 place-items-center rounded-full border border-line bg-surface text-ink-faint shadow-[0_1px_4px_rgba(26,35,50,0.08)] transition-colors hover:border-line-strong hover:text-ink-muted focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand"
      >
        {collapsed ? (
